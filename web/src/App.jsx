...
const isResetPasswordPage =
  ...
  enabled: false,
    ...
    width: Number(overrides.signatureWidth ?? signatureWidth) || 180,
height: Number(overrides.signatureHeight ?? signatureHeight) || 60,
...
const PREVIEW_TEMPLATE_PRESETS = {
  officialCircle: {
    shape: "circle",
    qr: { x: 0.5, y: 0.28, size: 0.1, anchor: "center" },
  },
  genericCircle: {
    shape: "circle",
...
const exportAuditPdf = () => {
  ...
  const html = `
    <html>
      <head>
        <title>eStamp Pro Audit Report</title>
        <style>
          ...
          <tbody>
            ${rows.map((a) => `
              <tr>
                ...
                <td>${a.displayUser}</td>
                <td>${a.displayRole}</td>
                <td>${a.displayAction}</td>
                <td>${a.displayFile}</td>
                <td class="${a.ok === false ? "failed" : "success"}">
                  ${a.displayStatus.replace("✅ ", "").replace("❌ ", "")}
                </td>
              </tr>
            `).join("")}
...
>
  ...
  onChange={(e) => changeTeamRole(member._id, e.target.value)}
>
                          <option value="user">User</option>
                          <option value="admin">Admin</option>
                          <option value="verifier">Verifier</option>
