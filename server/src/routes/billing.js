...
async function applySubscriptionToOrg({ customerId, subscription, fallbackStatus = "inactive" }) {
  ...
  org.billing = {
    ...
    last_event_id: org.billing?.last_event_id || "",
  };
  await org.save();
...
router.post("/checkout", requireAuth, async (req, res) => {
  try {
    ...
    const priceId = tier === "business" ? stripePriceBusiness : stripePricePro;

    if (!priceId) {
      return res.status(500).json({
        error: "stripe_price_missing",
        detail: tier === "business" ? "STRIPE_PRICE_BUSINESS is missing on the server" : "STRIPE_PRICE_PRO is missing on the server",
      });
    }
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: {
        ...
        email: me.email || "",
        tier: tier === "business" ? "business" : "pro",
      },
      subscription_data: {
        metadata: {
          org_id: String(org._id),
    ...
    switch (event.type) {
      case "checkout.session.completed": {
        ...
        if (customerId && subscriptionId) {
          ...
          await applySubscriptionToOrg({ customerId: String(customerId), subscription, fallbackStatus: "active" });
        }
        break;
      }
      ...
      case "customer.subscription.deleted": {
        ...
        if (customerId) {
          await applySubscriptionToOrg({ customerId: String(customerId), subscription, fallbackStatus: event.type === "customer.subscription.deleted" ? "canceled" : subscription.status });
        }
        break;
      }
      case "invoice.paid":
        ...
        if (customerId) {
          ...
          if (org) {
            org.billing = {
              ...
              subscription_status: event.type === "invoice.payment_failed" ? "past_due" : (org.billing?.subscription_status || "active"),
    ...
    if (["checkout.session.completed", "customer.subscription.created", "customer.subscription.updated", "customer.subscription.deleted", "invoice.paid", "invoice.payment_failed"].includes(event.type)) {
      ...
      if (customerId) {
        await Organization.updateOne(
          ...
          { $set: { "billing.last_event_id": event.id } }
        );
      }
    }
